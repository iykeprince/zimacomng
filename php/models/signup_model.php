...
class signup_model extends model{
    public function __construct(){
        ...
        if(Session::get('user_login') === TRUE){
            header('location: '.URL.'user/?redir=signup_oops');
        }
    }
    public function createUser($data){
        try{
            $referalEmail = $data['referal'];
            if($referalEmail != ""){
                $validateReferalEmail = $this->db->getRowCounts("SELECT * FROM users WHERE email='$referalEmail'");
                if($validateReferalEmail === 0){
                    return "invalid_referal";
                }else{
                    $referalInfo = $this->db->getItem("SELECT * FROM users WHERE email='$referalEmail'");
                    $referal_update_data = [
                        "wallet_balance" => '1'
                    ];
                    $referal_where = "email='$referalEmail'";
                    $this->db->update("users", $referal_update_data, $referal_where);
                }
            }
            $db_data = [
            ...
            $username = $data['username'];
            $email = $data['email'];
            $rowCount = $this->db->getRowCounts("SELECT * FROM users WHERE username='$username' OR email='$email'");
            if($rowCount > 0){
                return "exists";
            }else{
                $profileId = $this->db->insert('users', $db_data);
                $profile_data = [
                    "user_id" => $profileId,
                    "name" => $data['name'],            
                    "mobile" => $data['mobile'],
                ...
                try{
                    ...
                    $username = $data['username'];
                    $email = $data['email'];
                    $hash_id = base64_encode($profileId);
                    $mail = new PHPMailer;
                        ...
                        <td width="85" align="center" valign="bottom" style="font-family:Arial, Helvetica, sans-serif; font-size:16px; font-weight:bold; color:#FFF; padding-top:0px; margin-right: 5px;" mc:edit="bm1-03"><multiline><a href="#" style="text-decoration:none; color:#FFF;">Hi '.$username.'</a></multiline></td>
                          ...
                          <tr>
                            <td align="center" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                              ...
                              <tr>
                                <td align="center" valign="top"><table width="300" border="0" align="center" cellpadding="0" cellspacing="0" class="two-left-inner">
                                  ...
                                  <tr>
                                    <td align="center" valign="top"><table width="100" border="0" cellspacing="0" cellpadding="0">
                                      <tr>
                                        <td height="40" align="center" valign="middle" bgcolor="#133d55" style="font-family:Arial, Helvetica, sans-serif; font-size:18px; font-weight:bold; color:#FFF; -moz-border-radius: 40px; border-radius: 40px;" mc:edit="bm1-07"><multiline><a href="https://www.zima.com.ng/account/activate/'.$email.'?zima='.$hash_id.'" style="text-decoration:none; color:#FFF;">Activate</a></multiline></td>
                                      </tr>
                                    </table></td>
                                  </tr>
                                </table></td>
                              ...
                              <tr>
                                 <td align="center" valign="top" style="font-size:13px; line-height:23px;">&nbsp;If you cannot click on the button, copy the link below and paste it directly to the browser address bar <h4><a href="https://www.zima.com.ng/account/activate/'.$email.'?zima='.$hash_id.'">https://www.zima.com.ng/account/activate/'.$email.'?zima='.$hash_id.'</a></h4></td>
                              </tr>
                            </table></td>
                          </tr>
                          <tr>
    ...
    public function searchEmail($email){
        $count = $this->db->getRowCounts("SELECT * FROM users WHERE email='$email' ");
        if($count > 0){
            return "exists";
        }else{
            return "ok";
    ...
    public function searchUsername($username){
        $count = $this->db->getRowCounts("SELECT * FROM users WHERE username='$username' ");
        if($count > 0){
            return "exists";
        }else{
            return "ok";
